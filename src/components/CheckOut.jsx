...
function CheckOut() {
    ...
    const items = useSelector((state) => state.cart.items) //to get cart items stored in redux store

    // to dynamically update and store userInfo useState is used
    ...
    // to make changes to user info form field
    function handleInput(input, field) {
        setUserInfo({ ...userInfo, [field]: input });
    }

    const [ordered, setOrdered] = useState(false) //to control the state of the order status

    // handlePlaceOrder() is used to clear cart,set state of order to true at last to navigate back to home after placing order
    function handlePlaceOrder() {
        if (!userInfo.fullName || !userInfo.email || !userInfo.address || !userInfo.phone) { //this if block will check weather form field is empty 
    ...
    // if order is placed below will execute
    if (ordered == true) {
        return (
